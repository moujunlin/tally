...
async function refreshWeatherCache(db: any): Promise<any> {
  try {
    ...
    if (cached?.value) {
      ...
      if (parsed?.fetched_at && Date.now() - new Date(parsed.fetched_at).getTime() < WEATHER_CACHE_TTL_MS) {
        return parsed;
      }
    }
  } catch {}
  ...
  try {
    ...
    const city = encodeURIComponent(cfg.value);
    ...
    await db
      .from("system_config")
      .upsert({ key: "weather_cache", value: JSON.stringify(cacheData) }, { onConflict: "key" });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (resource === "checklist-logs") {
      // refresh weather cache in background on every checklist access
      refreshWeatherCache(db).catch(() => {});
    ...
    if (resource === "weather") {
      if (req.method === "GET") {
        const weather = await refreshWeatherCache(db);
        return json(weather || { error: "No weather data" });
      }
    }
    ...
    if (resource === "city") {
      if (req.method === "GET") {
        const { data } = await db
          ...
          .eq("key", "current_city")
          .single();
        return json({ city: data?.value || null });
      }
      if (req.method === "PATCH") {
        const body = await req.json();
